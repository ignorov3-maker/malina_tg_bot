...
const maxRequestBytes = Number(process.env.MAX_REQUEST_BYTES || 18 * 1024 * 1024);
...
async function startServer() {
  ...
  server.listen(port, host, () => {
    ...
    if (botToken) {
      ...
      dispatchQueuedDialogsToFreeManagers().catch((error) => {
        console.error("Queued dialog dispatch failed:", error.message);
      });
    }
  });
}
...
async function handleManagerStart(message, chatId) {
  ...
  if (alreadyManager && !isManagerBusy(chatId)) {
    await dispatchNextQueuedDialog(chatId, { notifyWhenEmpty: false });
  }
}
...
async function dispatchNextQueuedDialog(managerId, options = {}) {
  const notifyWhenEmpty = options.notifyWhenEmpty !== false;
  const store = readDialogStore();
  ...
  await writeDialogStore(store);
  const sent = await notifyAssignedManager(nextDialog, "", "new", getClientAttachments(nextDialog));

  if (!sent) {
    const latestStore = readDialogStore();
    const latestDialog = latestStore.leads.find((dialog) => dialog.id === nextDialog.id);

    if (latestDialog && String(latestDialog.assignedManagerId) === String(managerId)) {
      queueDialog(latestDialog);
      latestDialog.assignedManagerId = "";
      latestDialog.assignedAt = "";
      await writeDialogStore(latestStore);
    }
...
async function dispatchQueuedDialogsToFreeManagers() {
  for (const manager of getManagers()) {
    const store = readDialogStore();
    if (isManagerBusy(manager.id, store)) continue;

    const hasQueuedDialog = store.leads.some(
      (dialog) => dialog.status === "queued" && !dialog.assignedManagerId
    );
    if (!hasQueuedDialog) break;

    await dispatchNextQueuedDialog(manager.id, { notifyWhenEmpty: false });
  }
}
